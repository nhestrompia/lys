...
2. Identify the smallest user outcome being requested. Trace its real entry screen, controls,
   repeated states, terminal state, and observable acceptance criteria from code.
...
7. Run the app's ordinary tests, SDK compile/type-check, and a contract export test. Inspect the
   native accessibility tree when possible and confirm screen anchors do not hide child controls.
8. Report the exact flows added, auth mode, tests run, and any missing product semantics.
