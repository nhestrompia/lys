...
- **Malicious flow contract:** repository blueprints are data, not executable code. They use a closed schema, bounded loops, host-resolved selectors, and logical secret references. Destructive or external capabilities fail closed without a separate user approval; an agent cannot self-approve through tool arguments.
